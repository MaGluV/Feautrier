    ...
    class Feautrier
    {
        ...
        public Feautrier(double[][] dtauL, double[][] dtauU, double[][] dtaus, double[] mu, double[] source)
        {
            ...
            for (i = 1; i < this.t - 1; i++)
            {
                ...
                this.TauMu[i, 0] = -this.LUMtxCreate(mu, dt, dtl);
                //this.TauMu[i, 1] = new Matrix(this.m);
                this.TauMu[i, 1] = this.DiagMtxCreate(mu, dt, dtl, dtu);
                this.TauMu[i, 2] = -this.LUMtxCreate(mu, dt, dtu);
        ...
        private Matrix LUMtxCreate(double[] mu, double[] dtau, double[] dtau1)
        {
            ...
            for (i = 0; i < this.m; i++)
            {
                for (k = 0; k < this.m; k++)
                {
                    mtx[i, k] = (i == k) ? Math.Pow(mu[i], 2.0) / (dtau[i]*dtau1[i]) : 0; // Math.Pow(mu[i]/dtau, 2.0) : 0;
                }
            }
        ...
        private Matrix DiagMtxCreate(double[] mu, double[] dtau, double[] dtau1, double[] dtau2)
        {
            ...
            for (i = 0; i < this.m; i++)
            {
                for (k = 0; k < this.m; k++)
                {
                    mtx[i, k] = (i == k) ? 1.0 + Math.Pow(mu[i], 2.0)*(1.0/dtau1[i] + 1.0/dtau2[i])/dtau[i] : 0; // Math.Pow(mu[i]/dtau, 2.0) : 0;
                }
            }
            return new Matrix(mtx);
        }
        ...
        private Matrix NearBoundary(double[] mu, double[] dtau0)
        {
            ...
            for (i = 0; i < this.m; i++)
            {
                for (k = 0; k < this.m; k++)
                {
                    mtx[i, k] = (i == k) ? 0.5 + mu[i] / dtau0[i] + Math.Pow(mu[i] / dtau0[i], 2.0) : 0; //0.5*Math.Pow(dtau0, 2.0) + mu[i] * dtau0 + Math.Pow(mu[i], 2.0) : 0; //
                }
            }
            return new Matrix(mtx);
        }
        ...
        private Matrix FarBoundary(double[] mu, double[] dtauN)
        {
            ...
            for (i = 0; i < this.m; i++)
            {
                for (k = 0; k < this.m; k++)
                {
                    mtx[i, k] = (i == k) ? 0.5 + mu[i] / dtauN[i] + Math.Pow(mu[i] / dtauN[i], 2.0) : 0; //0.5 * Math.Pow(dtauN, 2.0) + mu[i] * dtauN + Math.Pow(mu[i], 2.0) : 0; //
                }
            }
            return new Matrix(mtx);
        }
